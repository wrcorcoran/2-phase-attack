...
def normalize_adj_tensor(adj, sparse=False):
    device = adj.device  # ✅ Get the device (CPU or GPU)

    if sparse:
        adj = to_scipy(adj)
        mx = normalize_adj(adj.tolil())
        return sparse_mx_to_torch_sparse_tensor(mx).to(device)  # ✅ Ensure it stays on the same device
    else:
        mx = adj + torch.eye(adj.shape[0], device=device)  # ✅ Ensure same device
        rowsum = mx.sum(1)
...
def likelihood_ratio_filter(node_pairs, modified_adjacency, original_adjacency, d_min, threshold=0.004):
    """
    Filter the input node pairs based on the likelihood ratio test proposed by Zügner et al. 2018.
    Ensures that only valid edges are considered for perturbation.
    """
    ...
    # First move tensor to CPU, then convert to numpy, and use bool instead of np.bool
    allowed_edges = new_ratios < threshold
    allowed_edges_cpu = allowed_edges.cpu().numpy()  # Move to CPU before using NumPy
    filtered_edges = node_pairs[allowed_edges_cpu.astype(bool)]  # Use bool instead of np.bool
    
    # Make sure allowed_mask is on the same device as modified_adjacency
    device = modified_adjacency.device
    allowed_mask = torch.zeros(modified_adjacency.shape, device=device)
