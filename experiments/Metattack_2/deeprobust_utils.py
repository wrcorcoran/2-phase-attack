    ...
    if sparse:
        ...
        if sp.issparse(features):
            if isinstance(features, torch.Tensor):
                features = features.float()  # Ensure it's a float tensor
            elif sp.issparse(features):
                features = torch.FloatTensor(features.toarray())  # Convert from SciPy sparse matrix
            else:
                features = torch.FloatTensor(features)  # Handle NumPy arrays
        else:
            features = torch.FloatTensor(features)
            
        if isinstance(adj, torch.Tensor):
            adj = adj.float()  # Ensure it's a float tensor
        elif sp.issparse(adj):
            adj = torch.FloatTensor(adj.toarray())  # Convert from SciPy sparse matrix
        else:
            adj = torch.FloatTensor(adj)  # Handle NumPy arrays
