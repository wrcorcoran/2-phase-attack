...
def load_model_and_edges(model_save_path, list_save_path, model, device):
    loaded_model_state_dict = torch.load(model_save_path)

    model = model.to(device)
    model.load_state_dict(loaded_model_state_dict)

    if not list_save_path:
        edges_to_add = None
    else:
...
def plot_results(dic, ptb_rate, title_one, title_two, title_three, t, is_reversed):
    ...
    plt.ylabel(t)
    plt.title(
        f"{t} vs # of edges inserted (ptb rate = {ptb_rate}), {title_one} method, {title_two} decision, {title_three} acceptance, {is_reversed}"
    )
    plt.legend()
