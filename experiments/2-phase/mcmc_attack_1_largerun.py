...
def collect_edges(model, data, device):
    # Run a single large Metattack instead of 10 smaller ones
    attacker = Metattack(data, device=device)
    attacker.setup_surrogate(
        model,
        labeled_nodes=data.train_mask,
    ...
    attacker.reset()
    attacker.attack(1.0)  # Increase perturbation rate to 100%

    # Collect added edges
    sorted_keys = list(attacker._added_edges.keys())
